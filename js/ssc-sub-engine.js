...
// does exact-case property lookups like EXAM_JSON.data[year][subject], so we
// must normalize to the real key, not just accept the user's casing).
...
// ── URL Filter Writer ─────────────────────────────────────────────────────────
// Mirrors currentFilters into the address bar as &filter=year=...,subject=...
...
function _updateUrlFilter() {
    try {
        const filterString = ['year', 'subject', 'topic', 'subtopic']
...
async function initExamEngine() {
    ...
    try {
        ...
        const matchedYear = _matchCaseInsensitive(years, currentFilters.year);
        if (matchedYear) {
            currentFilters.year = matchedYear;
        } else {
        ...
        const matchedSubject = _matchCaseInsensitive(subjectsForYear, currentFilters.subject);
        if (matchedSubject) {
        ...
        if (currentFilters.topic) {
            ...
            const topics = (subData && !Array.isArray(subData)) ? Object.keys(subData) : [];
            const matchedTopic = _matchCaseInsensitive(topics, currentFilters.topic);
            if (matchedTopic) {
        ...
        if (currentFilters.subtopic && currentFilters.topic) {
            ...
            const subtopics = (topicData && !Array.isArray(topicData)) ? Object.keys(topicData) : [];
            const matchedSubtopic = _matchCaseInsensitive(subtopics, currentFilters.subtopic);
...
function setupFilters(years) {
    ...
    if (subScroll) {
        subScroll.innerHTML = subjects.map(s => {
            ...
            return `<div class="pill-filter ${s === currentFilters.subject ? 'active' : ''}" onclick="setDeepFilter('subject', '${_escJs(s)}')">${s}<span class="pill-count">${count}</span></div>`;
        }).join('');
    }
    ...
    if (topicScroll && depth >= 1) {
        ...
        topicScroll.innerHTML = topics.map(t => {
            ...
            return `<div class="pill-filter ${t === currentFilters.topic ? 'active' : ''}" onclick="setDeepFilter('topic', '${_escJs(t)}')">${t}<span class="pill-count">${count}</span></div>`;
        }).join('');
    } else {
    ...
    if (depth === 2 && currentFilters.topic) {
        ...
        if (subtopicScroll && topicData && !Array.isArray(topicData)) {
            ...
            subtopicScroll.innerHTML = subtopics.map(st => {
                ...
                return `<div class="pill-filter ${st === currentFilters.subtopic ? 'active' : ''}" onclick="setDeepFilter('subtopic', '${_escJs(st)}')">${st}<span class="pill-count">${count}</span></div>`;
            }).join('');
        } else {
...
// ── Keyword Search — searches SUBJECT/TOPIC/SUBTOPIC keys across the whole
//    JSON (not mock titles). Typing "geometry" finds every "Subject: Topic"
//    combo whose topic name contains it; typing a subject name directly
//    (e.g. "math") returns that subject on its own. Clicking a result jumps
//    straight to that subject/topic/subtopic via the existing filter setters.
// ────────────────────────────────────────────────────────────────────────────
const SEARCH_RESULTS_LIMIT = 25;

function _searchKeys(query) {
    const q = String(query).trim().toLowerCase();
    if (!q || !EXAM_JSON) return [];

    const data = EXAM_JSON.data || {};
    const year = currentFilters.year;
    const subject = currentFilters.subject;
    const subData = (data[year] || {})[subject];
    const depth = _depth(subData);
    const results = [];

    if (depth === 0) return results; // no topics/subtopics under this subject

    Object.keys(subData).forEach(topic => {
        if (topic.toLowerCase().includes(q)) {
            results.push({ year, subject, topic, subtopic: '', label: topic });
        }

        if (depth === 2) {
            const topicData = subData[topic] || {};
            Object.keys(topicData).forEach(subtopic => {
                if (subtopic.toLowerCase().includes(q)) {
                    results.push({
                        year, subject, topic, subtopic,
...
function _renderSearchResults(matches, query) {
    ...
    if (!matches.length) {
        dropdown.innerHTML = `<div style="padding:12px 16px;color:#94a3b8;font-size:13px;">No matching topic found for "${query}"</div>`;
    ...
    dropdown.innerHTML = matches.map(m => `
        <div class="search-result-item" style="padding:10px 16px;cursor:pointer;border-bottom:1px solid #f1f5f9;font-size:14px;color:#1e293b;"
             ...
             onclick="_selectSearchResult('${_escJs(m.year)}','${_escJs(m.subject)}','${_escJs(m.topic)}','${_escJs(m.subtopic)}')">
            ${m.label}
        </div>`).join('');
    dropdown.style.display = 'block';
}

function _selectSearchResult(year, subject, topic, subtopic) {
    currentFilters.year = year;
    currentFilters.subject = subject;
    currentFilters.topic = topic || '';
    currentFilters.subtopic = subtopic || '';
    saveFilters();
...
function _wireSearchInput() {
    ...
    input.addEventListener('input', () => {
        ...
        _renderSearchResults(_searchKeys(q), q);
    });
...
function _buildScoreAndFooterHtml(item, isSubmitted, startLink) {
    ...
    const shareUrlAbs = new URL(startLink, window.location.href).href;
...
// NEW
const startLink = `../${config.subject_link}?${item.linkParam}`;
const scoreFooterHtml = _buildScoreAndFooterHtml(item, isSubmitted, startLink);
const cardHtml = `<div class="mock-card${item.type === 'paid' ? ' is-paid' : ''}">
            <div class="card-clip">
                <div class="card-row">
    ...
    grid.innerHTML = buildGridHtml(cardEntries);
    document.getElementById('grid-sync').innerText = "";
}
...
function buildGridHtml(cardEntries) {
    if (!cardEntries.length) {
        return `<div class="empty-state">
                    ...
                    <div class="empty-title">No mocks here yet</div>
                    <div class="empty-sub">Try a different topic, or clear your search to browse all mocks.</div>
                </div>`;
    }
